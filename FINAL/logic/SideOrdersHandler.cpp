...
void SideOrdersHandler::make_side_order(string name, string price) {
    got_list();
    ...
    try{
        ...
        sideorder.set_name(name);
        sideorder.set_price(price);
    }
    ...
    if(validate_name(sideorder.get_side_orders())) {
        throw InvalidName();
    }
    side_order_repo.write(sideorder);
    side_order_vector.push_back(sideorder);
}
