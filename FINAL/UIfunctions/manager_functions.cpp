...
void make_new_menu_pizza(PizzaHandler& pizzahandler, ToppingsHandler& toppingshandler) {
    ...
    do{
        try{
            cout << "Price for these toppings is: "
                 << pizzahandler.calc_price(pizza) << endl;
            cout << "What price should be added to the base?: ";
            cin >> price;
...
void archive_orders(OrderHandler& orderhandler) {
    cout << "Do you wish to archive the orders for " << orderhandler.get_location() << " (y/n)? ";
    string ans;
    cin >> ws;
    getline(cin, ans);
    try{
